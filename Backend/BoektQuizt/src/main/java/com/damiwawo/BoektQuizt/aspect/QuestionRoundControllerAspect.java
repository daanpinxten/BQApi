...
@Aspect
@Configuration
public class QuestionRoundControllerAspect {

    private LoggerHelper loggerHelper = new LoggerHelper();

    @Before("execution(* com.damiwawo.BoektQuizt.controller.QuestionRoundController.*(..) )")
    public void checkAccessForAllMethodsOnController(JoinPoint joinPoint){
        loggerHelper.LogBefore(joinPoint);
    }

    @After("execution(* com.damiwawo.BoektQuizt.controller.QuestionRoundController.*(..) )")
