...
@Aspect
@Configuration
public class QuizControllerAspect {

    private LoggerHelper loggerHelper = new LoggerHelper();

    @Before("execution(* com.damiwawo.BoektQuizt.controller.QuizController.*(..) )")
    public void checkAccessForAllMethodsOnController(JoinPoint joinPoint){
        loggerHelper.LogBefore(joinPoint);
    }

    @After("execution(* com.damiwawo.BoektQuizt.controller.QuizController.*(..) )")
