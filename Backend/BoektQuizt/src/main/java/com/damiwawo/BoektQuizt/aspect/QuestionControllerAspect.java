...
@Aspect
@Configuration
public class QuestionControllerAspect {

    private LoggerHelper loggerHelper = new LoggerHelper();

    @Before("execution(* com.damiwawo.BoektQuizt.controller.QuestionController.*(..) )")
    public void checkAccessForAllMethodsOnController(JoinPoint joinPoint){
        loggerHelper.LogBefore(joinPoint);
    }

    @After("execution(* com.damiwawo.BoektQuizt.controller.QuestionController.*(..) )")
